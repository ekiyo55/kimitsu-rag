...
書籍『AIに機密情報を持たせる方法 ― 外に出さずに読ませる、自分だけのローカルAI』（江藤 潔）で作る、自分のPC一台で完結するRAG（検索拡張生成）アプリです。自分の資料（Word / PDF）を検索し、その内容だけを根拠に日本語で答え、根拠にした資料名とページを示します。資料も質問も、PCの外には一切送りません。
...
- 初回起動時に Windows のファイアウォールが python.exe の通信許可を尋ねてきたら、**プライベートネットワークのみ**許可してください（パブリックは許可しない）
...
- モデルや Ollama、ライブラリを更新したら `python replay_history.py 5` で、履歴にある直近の質問を再実行し、当時の答えと今の答えを見比べられます
...
MIT License（`LICENSE` を参照）。サンプル資料の著作権は各出典元にあります。
